...
void MachineVerifier::OnEnterSystemUpdate(SystemBase& base) {
	...
	if (!ext_sys && (ext_sys = CastPtr<ExtSystem>(&base))) {
		...
		ext_sys->WhenEnterExtComponentForward << THISBACK(OnEnterExtComponentForward);
		ext_sys->WhenEnterFwdScopeForward << THISBACK(OnEnterFwdScopeForward);
		
		ext_sys->WhenLeaveOnceForward << THISBACK(OnLeaveOnceForward);
		ext_sys->WhenLeaveExtComponentForward << THISBACK(OnLeaveExtComponentForward);
...
void MachineVerifier::OnEnterExtComponentForward(ExtComponent* c) {
	RTLOG("MachineVerifier::OnEnterExtComponentForward " << HexStr((void*)c));
...
void MachineVerifier::OnEnterFwdScopeForward(FwdScope& f) {
	...
	c = 0;
	vep = 0;
	if ((c = CastPtr<ExtComponent>(f.GetCurrent()))) {
...
void MachineVerifier::OnEnterStorePacket(ComponentExtBase& b, Packet& p) {
...
void MachineVerifier::OnLeaveExtComponentForward() {
	RTLOG("MachineVerifier::OnLeaveExtComponentForward");
