...
void ShootingInteractionSystem::Register(const LinkedList<EntityRef>& entities)
{
    ...
    const mat4 model_to_controller_rotation = make_mat4_from_yaw_pitch_roll(DirectX::XMConvertToRadians(180), DirectX::XMConvertToRadians(70), 0.0f);
    const mat4 model_to_controller_translation = make_mat4_translation(0, 0.05f, 0.0f);
...
void ShootingInteractionSystem::OnSourcePressed(const SpatialInteractionSourceEventArgs& args)
{
    ...
    {
        auto shooting = std::get<ShootingComponent*>(*enabled_entity);

        if (args.PressKind() == SpatialInteractionPressKind::Select)
        {
            const mat4 barrel_to_world = shooting->barrel_to_ctrl * shooting->gun->Get<Transform>()->GetMatrix();

            const vec3 position = mat4_util::position(barrel_to_world);
            const quaternion orientation = make_quaternion_from_rotation_matrix(barrel_to_world);

            const vec3 forward = mat4_util::forward(barrel_to_world);
...
void ShootingInteractionSystem::OnSourceUpdated(const SpatialInteractionSourceEventArgs& args)
{
    if (auto enabled_entity = TryGetEntityFromSource(args.State().Source()))
    {
        ...
        shooting->gun->Get<PbrRenderable>()->AlphaMultiplier = (should_render_controller) ? std::make_optional(0.25f) : std::nullopt;
    }
}
