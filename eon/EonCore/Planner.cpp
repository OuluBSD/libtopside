...
bool WorldState::Append(const WorldState& ws, LinkedList<Statement>& ret_list) {
	for (const Statement& ret : ret_list) {
		String atom = ret.id.ToString();
		if (!ret.value.IsEmpty()) {
			const Value& v = *ret.value;
			if (v.type == Value::VAL_STRING)
				Set(atom, v.str);
			else if (v.type == Value::VAL_BOOLEAN)
				Set(atom, v.b ? "true" : "false");
			else if (v.type == Value::VAL_ID)
...
int ActionPlanner::GetAddAtom(const IdPath& id) {
...
void ActionPlanner::GetPossibleStateTransition(Node<ActionNode>& n, Array<WorldState*>& dest, Vector<double>& action_costs)
{
	...
	acts.SetCount(0);
	Eon::Factory::GetComponentActions(src, acts);
