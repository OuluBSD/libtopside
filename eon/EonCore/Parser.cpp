...
bool Id::operator==(const IdPath& id) const {
...
void ChainDefinition::GetSubChainPointers(LinkedList<ChainDefinition*>& ptrs) {
...
bool Parser::Parse(CompilationUnit& cunit) {
...
bool Parser::ParseGlobalScope(GlobalScope& list) {
	if (IsId("chain")) {
...
bool Parser::ParseMachine(MachineDefinition& mach) {
	PASS_ID("machine")
...
bool Parser::ParseMachineScope(MachineDefinition& mach) {
	PASS_CHAR('{')
...
bool Parser::ParseState(State& state) {
	PASS_ID("state")
...
bool Parser::ParseStateScope(State& state) {
	PASS_CHAR('{')
...
bool Parser::ParseLoop(LoopDefinition& def) {
	PASS_ID("loop")
...
bool Parser::ParseChain(ChainDefinition& def) {
	PASS_ID("chain")
...
bool Parser::ChainScope(ChainDefinition& def) {
	PASS_CHAR('{')
...
bool Parser::ParseLoopScope(LoopDefinition& def) {
	PASS_CHAR('{')
...
bool Parser::ParseStmt(Statement& stmt) {
	if (!ParseId(stmt.id))
		return false;
...
bool Parser::ParseStmtArguments(Statement& stmt) {
	PASS_CHAR('{')
...
bool Parser::ParseValue(Value& v) {
	if (Id("true")) {
		...
		v.type = Value::VAL_INT;
		v.i = ReadInt();
		return true;
	}
	else if (IsNumber()) {
		v.type = Value::VAL_DOUBLE;
		v.f = ReadNumber();
		return true;
	}
	else if (IsChar('{')) {
		v.type = Value::VAL_CUSTOMER;
...
bool Parser::ParseReturnStmt(Statement& stmt) {
	PASS_ID("return")
	if (!ParseId(stmt.id))
		return false;
	if (Char(':')) {
...
bool Parser::ParseRequirementStmt(IdPath& id) {
	PASS_ID("req")
	if (!ParseId(id))
		return false;
	PASS_CHAR(';')
