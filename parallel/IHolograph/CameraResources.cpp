    ...
    )
{
    ...
    Holo::CheckResult(surface.as<::Windows::Graphics::DirectX::Direct3D11::IDirect3DDxgiInterfaceAccess>()->GetInterface(IID_PPV_ARGS(&cameraBackBuffer)));
    ...
    if (m_d3dBackBuffer.Get() != cameraBackBuffer.Get())
    {
        ...
        vec2 currentSize = m_holographicCamera.RenderTargetSize();
...
void GfxCamResources::ReleaseResourcesForBackBuffer(const GfxDevResources* pDeviceResources)
{
    ...
    NativeRenderTargetViewRef nullViews[D3D11_SIMULTANEOUS_RENDER_TARGET_COUNT] = { nullptr };
...
void GfxCamResources::CommitDirect3D11DepthBuffer(HolographicCameraRenderingParameters const& renderingParameters) const
{
    // Direct3D interop APIs are used to provide the buffer to the WinRT API.
    ComPtr<IDXGIResource1> depthStencilResource;
    Holo::CheckResult(m_d3dDepthStencil.As(&depthStencilResource));

    ComPtr<IDXGISurface2> depthDxgiSurface;
    Holo::CheckResult(depthStencilResource->CreateSubresourceSurface(0, &depthDxgiSurface));

    NativeInspectableRef inspectableSurface;
    TODO //Holo::CheckResult(CreateDirect3D11SurfaceFromDXGISurface(depthDxgiSurface.Get(), winrt::put_abi(inspectableSurface)));
