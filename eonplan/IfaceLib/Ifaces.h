...
PKG(Graphics, Gfx) {
	...
	PKG_IFACE {
		...
		FUNCTION1R(CreateShader, bool, ShaderType, NativeShader&)
		FUNCTION2V(ShaderSource, NativeShader&, String)
		FUNCTION1R(CompileShader, bool, NativeShader& s);
		FUNCTION1R(GetLastErrorS, String, NativeShader& s);1
		...
		FUNCTION4R(String GetActiveUniform(NativeProgram& prog, int i, int* size_out=0, int* type_out=0);
		...
		FUNCTION1R(bool GenTexture(NativeFrameBuffer& fb);
		...
		vBindVertexBuffer(NativeVertexBuffer& vbo);
