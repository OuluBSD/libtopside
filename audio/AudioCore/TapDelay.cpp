...
TapDelay::TapDelay( const Vector<unsigned long>& taps, unsigned long max_delay ) {
	if ( max_delay < 1 ) {
		LOG("TapDelay::TapDelay: max_delay must be > 0!\n");
		HandleError( AudioError::FUNCTION_ARGUMENT );
	}

	for ( unsigned int i = 0; i < taps.GetCount(); i++ ) {
		if ( taps[i] > max_delay ) {
...
void TapDelay::SetMaximumDelay( unsigned long delay ) {
	...
	for ( unsigned int i = 0; i < delays_.GetCount(); i++ ) {
		if ( delay < delays_[i] ) {
...
void TapDelay::SetTapDelays( const Vector<unsigned long>& taps ) {
	for ( unsigned int i = 0; i < taps.GetCount(); i++ ) {
	...
	if ( taps.GetCount() != out_point_.GetCount() ) {
		...
		last_frame_.SetCount( 1, (unsigned int)taps.GetCount(), 0.0 );
	}
