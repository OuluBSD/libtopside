...
MediaStream* MediaDevice::FindOpenDevice() {
...
MediaDevice& MediaDeviceManager::GetAddVirtual(String path) {
	for(MediaDevice& v : virtuals)
		if (v.GetPath() == path)
			return v;
	MediaDevice& v = virtuals.Add();
	v.path = path;
	return v;
}
...
void MediaDeviceManager::Refresh() {
	...
	for (int i = 0; i < 10; i++) {
		...
		if (fd != -1) {
			MediaDevice& dev = devs.Add();
			
			dev.path = path;
			
			MediaCaptureDevice* cap = 0;
