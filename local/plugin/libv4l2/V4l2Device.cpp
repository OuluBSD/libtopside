...
void V4l2Device::queryFormat()
{
	...
	if (0 == ioctl(m_fd,VIDIOC_G_FMT,&fmt)) 
	{
		...
		V4L_LOG(DEBUG) << m_params.m_devName << ":" << fourcc(m_format) << " size:" << m_width << "x" << m_height << " bufferSize:" << m_bufferSize;
	}
}
...
bool V4l2Device::init(unsigned int mandatoryCapabilities)
{
		...
		{
			V4L_LOG(ERROR) << "Cannot init device:" << m_params.m_devName;
		}
	}
	else
	{
...
int V4l2Device::initdevice(const char *dev_name, unsigned int mandatoryCapabilities)
{
	...
	if (m_fd < 0) 
	{
		V4L_LOG(ERROR) << "Cannot open device:" << m_params.m_devName << " " << strerror(errno);
		this->close();
		return -1;
	}
...
int V4l2Device::checkCapabilities(int fd, unsigned int mandatoryCapabilities)
{
	...
	{
		V4L_LOG(ERROR) << "Cannot get capabilities for device:" << m_params.m_devName << " " << strerror(errno);
		return -1;
	}
	V4L_LOG(NOTICE) << "driver:" << cap.driver << " capabilities:" << std::hex << cap.capabilities <<  " mandatory:" << mandatoryCapabilities << std::dec;
		
	if ((cap.capabilities & V4L2_CAP_VIDEO_OUTPUT))  V4L_LOG(NOTICE) << m_params.m_devName << " support output";
	if ((cap.capabilities & V4L2_CAP_VIDEO_CAPTURE)) V4L_LOG(NOTICE) << m_params.m_devName << " support capture";

	if ((cap.capabilities & V4L2_CAP_READWRITE))     V4L_LOG(NOTICE) << m_params.m_devName << " support read/write";
	if ((cap.capabilities & V4L2_CAP_STREAMING))     V4L_LOG(NOTICE) << m_params.m_devName << " support streaming";

	if ((cap.capabilities & V4L2_CAP_TIMEPERFRAME))  V4L_LOG(NOTICE) << m_params.m_devName << " support timeperframe"; 
	
	if ( (cap.capabilities & mandatoryCapabilities) != mandatoryCapabilities )
	{
		V4L_LOG(ERROR) << "Mandatory capability not available for device:" << m_params.m_devName;
		return -1;
	}
...
int V4l2Device::configureFormat(int fd, unsigned int format, unsigned int width, unsigned int height)
{
	...
	fmt.type                = m_deviceType;
        ...
        {
		V4L_LOG(ERROR) << m_params.m_devName << ": Cannot get format " << strerror(errno);
		return -1;
	}
	if (width != 0) {
	...
	{
		V4L_LOG(ERROR) << m_params.m_devName << ": Cannot set format:" << fourcc(format) << " " << strerror(errno);
		return -1;
	}			
	if (fmt.fmt.pix.pixelformat != format) 
	{
		V4L_LOG(ERROR) << m_params.m_devName << ": Cannot set pixelformat to:" << fourcc(format) << " format is:" << fourcc(fmt.fmt.pix.pixelformat);
		return -1;
	}
	if ((fmt.fmt.pix.width != width) || (fmt.fmt.pix.height != height))
	{
		V4L_LOG(WARN) << m_params.m_devName << ": Cannot set size to:" << width << "x" << height << " size is:"  << fmt.fmt.pix.width << "x" << fmt.fmt.pix.height;
	}
	...
	V4L_LOG(NOTICE) << m_params.m_devName << ":" << fourcc(m_format) << " size:" << m_width << "x" << m_height << " bufferSize:" << m_bufferSize;
...
int V4l2Device::configureParam(int fd, int fps)
{
	if (fps!=0)
	{
		...
		{
			V4L_LOG(WARN) << "Cannot set param for device:" << m_params.m_devName << " " << strerror(errno);
		}
	
		V4L_LOG(NOTICE) << "fps:" << param.parm.capture.timeperframe.numerator << "/" << param.parm.capture.timeperframe.denominator;
		V4L_LOG(NOTICE) << "nbBuffer:" << param.parm.capture.readbuffers;
	}
