...
{
	V4L_LOG(NOTICE) << "Device " << m_params.m_devName;
	...
	if (-1 == ioctl(m_fd, VIDIOC_REQBUFS, &req)) 
	{
		if (EINVAL == errno) 
		{
			V4L_LOG(ERROR) << "Device " << m_params.m_devName << " does not support memory mapping";
			success = false;
		} 
		else 
		{
	...
	{
		V4L_LOG(NOTICE) << "Device " << m_params.m_devName << " nb buffer:" << req.count;
		...
		for (n_buffers = 0; n_buffers < req.count; ++n_buffers) 
		{
			...
			{
				V4L_LOG(INFO) << "Device " << m_params.m_devName << " buffer idx:" << n_buffers << " size:" << buf.length << " offset:" << buf.m.offset;
...
size_t V4l2MmapDevice::readInternal(char* buffer, size_t bufferSize)
{
	...
	if (n_buffers > 0)
	{
		...
		else if (buf.index < n_buffers)
		{
			...
			{
				size = bufferSize;
				V4L_LOG(WARN) << "Device " << m_params.m_devName << " buffer truncated available:" << bufferSize << " needed:" << buf.bytesused;
			}
...
size_t V4l2MmapDevice::writeInternal(char* buffer, size_t bufferSize)
{
	...
	if (n_buffers > 0)
	{
		...
		else if (buf.index < n_buffers)
		{
			...
			if (size > buf.length)
			{
				V4L_LOG(WARN) << "Device " << m_params.m_devName << " buffer truncated available:" << buf.length << " needed:" << size;
				size = buf.length;
			}
...
size_t V4l2MmapDevice::writePartialInternal(char* buffer, size_t bufferSize)
{
	...
	if ((n_buffers > 0) && m_partialWriteInProgress)
	{
		if (m_partialWriteBuf.index < n_buffers)
		{
			...
			{
				V4L_LOG(WARN) << "Device " << m_params.m_devName << " buffer truncated available:" << m_partialWriteBuf.length << " needed:" << new_size;
