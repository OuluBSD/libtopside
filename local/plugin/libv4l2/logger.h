...
#define V4L_LOG(__level)  log4cpp::Category::getRoot() << log4cpp::Priority::__level << __FILE__ << ":" << __LINE__ << "\n\t" 
...
inline void initLogger(int verbose)
{
	...
	V4L_LOG(INFO) << "level:" << log4cpp::Priority::getPriorityName(log.getPriority()); 
}
#else
...
#include <iostream>
extern int LogLevel;
#define V4L_LOG(__level) if (__level<=LogLevel) std::cout << "\n[" << #__level << "] " << __FILE__ << ":" << __LINE__ << "\n\t" 
