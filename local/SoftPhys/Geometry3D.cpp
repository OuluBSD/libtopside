...
Point ClosestPoint(const AABB& aabb, const Point& point) {
	...
	result[1] = (result[1] < min[0]) ? min[1] : result[1];
	result[2] = (result[2] < min[0]) ? min[2] : result[2];

	result[0] = (result[0] > max[0]) ? max[0] : result[0];
	result[1] = (result[1] > max[0]) ? max[1] : result[1];
	result[2] = (result[2] > max[0]) ? max[2] : result[2];
	...
	if (node->children != 0 && node->numTriangles > 0) {
		for (int i = 0; i < 8; ++i) { // For each child
			...
			for (int j = 0; j < node->numTriangles; ++j) {
				const Triangle& t = model.triangles[node->triangles[j]];
...
Ray GetPickRay(const vec2& viewportPoint, const vec2& viewportOrigin, const vec2& viewportSize, const mat4& view, const mat4& projection) {
	vec3 nearPoint(viewportPoint[0], viewportPoint[1], 0.0f);
	vec3 farPoint(viewportPoint[0], viewportPoint[1], 1.0f);
...
CollisionManifold FindCollisionFeatures(const OBB& A, const Sphere& B) {
	...
	result.colliding = true;
	result.contacts.Add(closestPoint + (outsidePoint - closestPoint) * 0.5f);
