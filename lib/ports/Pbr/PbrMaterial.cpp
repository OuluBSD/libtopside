...
Material::Material(Pbr::Resources const& pbr_res)
{
    ...
    Internal::ThrowIfFailed(pbr_res.GetDevice()->CreateBuffer(&constantBufferDesc, nullptr, &m_constantBuffer));
    ...
    Internal::ThrowIfFailed(pbr_res.GetDevice()->CreateBlendState(&blendStateDesc, &m_blendState));
}

std::shared_ptr<Material> Material::Clone(Pbr::Resources const& pbr_res) const
{
...
/* static */
std::shared_ptr<Material> Material::CreateFlat(const Resources& pbr_res, CXMVECTOR baseColorFactor, float roughnessFactor /* = 1.0f */, float metallicFactor /* = 0.0f */, CXMVECTOR emissiveFactor /* = XMFLOAT3(0, 0, 0) */)
{
    std::shared_ptr<Material> material = std::make_shared<Material>(pbr_res);
    ...
    material->SetTexture(ShaderSlots::BaseColor, pbr_res.CreateSolidColorTexture(XMVECTORF32{ 1, 1, 1, 1 }).Get(), defaultSampler.Get());
    material->SetTexture(ShaderSlots::MetallicRoughness, pbr_res.CreateSolidColorTexture(XMVECTORF32{ 1, 1, 1, 1 }).Get(), defaultSampler.Get());
    material->SetTexture(ShaderSlots::Occlusion, pbr_res.CreateSolidColorTexture(XMVECTORF32{ 1, 1, 1, 1 }).Get(), defaultSampler.Get()); // No occlusion.
    material->SetTexture(ShaderSlots::Normal, pbr_res.CreateSolidColorTexture(XMVECTORF32{ 0.5f, 0.5f, 1, 1 }).Get(), defaultSampler.Get()); // Flat normal.
    material->SetTexture(ShaderSlots::Emissive, pbr_res.CreateSolidColorTexture(XMVECTORF32{ 1, 1, 1, 1 }).Get(), defaultSampler.Get());
