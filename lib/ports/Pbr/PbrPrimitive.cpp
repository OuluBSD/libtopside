...
Primitive::Primitive(Pbr::Resources const& pbr_res, const Pbr::PrimitiveBuilder& primitiveBuilder, std::shared_ptr<Pbr::Material> material)
    : Primitive(
        (UINT)primitiveBuilder.Indices.size(),
        CreateIndexBuffer(pbr_res.GetDevice().Get(), primitiveBuilder).Get(),
        CreateVertexBuffer(pbr_res.GetDevice().Get(), primitiveBuilder).Get(),
        std::move(material))
{
...
{
    return Primitive(m_indexCount, m_indexBuffer.Get(), m_vertexBuffer.Get(), m_material->Clone(pbr_res));
}
