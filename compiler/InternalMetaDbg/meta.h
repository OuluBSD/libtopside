...
public:
	void init(String s) {
		...
		std::string result = s.Begin();//(it, eos);
...
#define bt(label)  LOG("BT " #label); if (lex.is_match()) { goto label; }
#define bf(label)  if (!lex.is_match()) { goto label; }
#define set        lex.clear();
#define be         if (!lex.is_match()) { std::exit(1); }
#define out        std::cout << "\n";
#define adr(label) CONSOLE_APP_MAIN { lex.init(LoadFile(GetDataFile("input.txt"))); label(); }
#define cll(fun)   fun();
