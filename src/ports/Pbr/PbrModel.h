...
struct Model
{
    ...
    // Render the model.
    void Render(Pbr::Resources const& pbr_res, _In_ ID3D11DeviceContext3* context) const;

    // Remove all primitives.
    void Clear();

    // Create a clone of this model.
    std::shared_ptr<Model> Clone(Pbr::Resources const& pbr_res) const;
...
private:
    ...
    void UpdateTransforms(Pbr::Resources const& pbr_res, _In_ ID3D11DeviceContext3* context) const;
