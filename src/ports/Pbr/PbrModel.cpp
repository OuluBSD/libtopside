...
void Model::Render(Pbr::Resources const& pbr_res, _In_ ID3D11DeviceContext3* context) const
{
...
std::shared_ptr<Model> Model::Clone(Pbr::Resources const& pbr_res) const
{
...
void Model::UpdateTransforms(Pbr::Resources const& pbr_res, _In_ ID3D11DeviceContext3* context) const
{
    ...
    if (newTotalModifyCount != TotalModifyCount || m_modelTransformsStructuredBuffer == nullptr)
    {
        if (m_modelTransformsStructuredBuffer == nullptr) // The structured buffer is reset when a Node is added.
        {
            ...
            Internal::ThrowIfFailed(pbr_res.GetDevice()->CreateBuffer(&desc, nullptr, &m_modelTransformsStructuredBuffer));
            ...
            Internal::ThrowIfFailed(pbr_res.GetDevice()->CreateShaderResourceView(m_modelTransformsStructuredBuffer.Get(), &srvDesc, &m_modelTransformsResourceView));
        }
