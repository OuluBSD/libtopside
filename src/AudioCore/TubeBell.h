#ifndef _AudioCore_TUBEBELL_H
#define _AudioCore_TUBEBELL_H
...
inline double TubeBell::Tick( unsigned int ) {
	double temp, temp2;
...
inline AudioFrames& TubeBell::Tick( AudioFrames& frames, unsigned int channel ) {
	unsigned int channel_count = last_frame_.GetChannelCount();
	#if defined(flagDEBUG)

	if ( channel > frames.GetChannelCount() - channel_count ) {
		LOG("TubeBell::Tick(): channel and AudioFrames arguments are incompatible!");
