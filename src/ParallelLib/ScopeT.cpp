...
template <class Dim>
ScopeT<Dim>::ScopeT() {
	this->SetDimensions(Dim::GetDefaultScopeDimensions());
...
void ScopeT<Dim>::AddInterface(InterfaceProxy& iface) {
	...
	int offset = i * 30;
	h.SetDimensions(Dim::GetDefaultHandleDimensions(offset));
...
void ScopeT<Dim>::MoveHandle(Point pt, int handle_id) {
	...
	Point tl = h.GetFrameBox().TopLeft();
	h.SetFrameBox(Box(tl.x + pt.x, tl.y + pt.y, sz.cx, sz.cy));
}
...
void ScopeT<Dim>::FocusHandlePos(int handle_pos) {
	if (handle_pos < 0 || handle_pos >= handles.GetCount()) return;
	...
	if (!h.IsMaximized()) {
		if (maximize_all) MaximizeHandle(handles.GetKey(handle_pos));
	} else {
		Sz sz(this->GetFrameSize());
		Handle& h = handles[handle_pos];
		h.SetFrameBox(BoxC(0, 0, sz.cx, sz.cy));
	}
...
void ScopeT<Dim>::SetHandleMaximized(Handle& h, bool b) {
	...
	if (b) {
		...
		if (h_size.cx > sz.cx ||h_size.cy > sz.cy) {
			h.SetFrameRect(RectC(10, 10, sz.cx-20, sz.cy-20));
		}
	}
}
...
void ScopeT<Dim>::FocusPrevious() {
	...
	while (handle_pos >= count) handle_pos--;
	for(int i = 0; i < count; i++) {
		if (handle_pos <= -1) handle_pos = count - 1;
...
bool ScopeT<Dim>::Key(dword key, int count) {
	...
	if (key == (K_CTRL|K_TAB)) {
		active_pos++;
		if (active_pos >= handles.GetCount()) active_pos = 0;
		FocusHandlePos(active_pos);
		return true;
	}
	if (key == (K_CTRL|K_SHIFT|K_TAB)) {
		active_pos--;
		if (active_pos < 0) active_pos = handles.GetCount()-1;
		FocusHandlePos(active_pos);
		return true;
	}
	return false;
	...
	for (GeomInteraction* c : this->sub) {
		...
		if (c2) {
			...
			if (r.IsEmpty()) {
				...
				r = BoxC(10, 10, 320, 240);
				c2->SetFrameBox(r);
			}
		}
	}
	
	// Resize active Handle if it is maximised and ScopeT<Dim> area size changes (TopContainer area changes usually)
	if (maximize_all) {
...
void ScopeT<Dim>::OrderTileHandles() {
	...
	for(int i = 0; i < cols; i++) {
		...
		for(int j = 0; j < rows_i; j++) {
			...
			h.SetFrameBox(BoxC(x, y, x_step, yi_step));
			h.SetMaximized(false);
			pos++;
		}
	}
...
void ScopeT<Dim>::OrderTileHandlesVert() {
	...
	for(int i = 0; i < handles.GetCount(); i++) {
		Handle& h = handles[i];
		h.SetFrameBox(BoxC(0, i * y_step, sz.cx, y_step));
		h.SetMaximized(false);
	}
}
